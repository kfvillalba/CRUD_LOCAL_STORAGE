...
const LeerProductosDB = () => {
    ...
    if (arrayProductos == null) {
        ...
        arrayProductos.forEach(element => {            
            ...
            listaProductosUI.innerHTML += `<div class="alert alert-primary" role="alert">
            <i class="material-icons float-start me-3">
                shopping_bag
                </i>
                Codigo:<b>${element.codProducto}</b> <span class="float-none"> 
                Nombre:${element.nombreProducto}                 
                Valor:${element.valorProducto}                  
                </span>
...
const ComprarProductosDB = (codProducto) => {
    ...
    indexArray = arrayCarrito.findIndex((producto) => producto.codProducto === codProducto)
    if (indexArray === -1) {
...
const pintarCarrito = () => {
    ...
    if (arrayCarrito == null) {
        ...
        arrayCarrito.forEach(producto => {
            ...
            templateCarrito.querySelectorAll('td')[1].textContent = producto.CantidadComprar
            templateCarrito.querySelector('.btn-info').dataset.id = producto.codProducto
            templateCarrito.querySelector('.btn-danger').dataset.id = producto.codProducto
            templateCarrito.querySelector('span').textContent = producto.CantidadComprar * producto.valorProducto
...
const btnAccion = e =>{
    let indexArray = arrayCarrito.findIndex((producto) => producto.codProducto === e.target.dataset.id)
...
document.addEventListener('DOMContentLoaded', LeerProductosDB(),pintarCarrito());
